...
func userGetRecordsInitDB(db *sql.DB) error {
	...
	for rows.Next() {
		...
		var user userType
		err = rows.Scan(&user.Email, &user.Name, &user.Gender, &user.BirthYear, &user.AccessKey, &user.Type)
		...
		userX.birthYear = user.BirthYear
...
func userGetOneRecordDB(db *sql.DB, ID string) (*userType, error) {
	...
	for row.Next() {
		...
		err = row.Scan(&user.Email, &user.Name, &user.Gender, &user.BirthYear, &user.AccessKey, &user.Type)
...
func userInsertRecordDB(db *sql.DB, p *userType) {
	...
	row, err := db.Query("INSERT INTO USERS VALUES (?, ?, ?, ?, ?, ?)",
		p.Email, p.Name, p.Gender, p.BirthYear, p.AccessKey, p.Type)
	if err != nil {
		panic(err.Error())
	}
	defer row.Close()
	...
	var userX userT
	userX.email = p.Email
	userX.gender = p.Gender
	userX.birthYear = p.BirthYear
...
func userUpdateRecordDB(db *sql.DB, user mapInterface, ID string) error {
	...
	if v, ok := user["gender"]; ok {
		userTemp.Gender = v.(string)
		if err != nil {
			panic(err.Error())
		}
	}

	if v, ok := user["birthyear"]; ok {
		userTemp.BirthYear = (int)(v.(float64))
		if err != nil {
			panic(err.Error())
		}
	}
	...
	row, err := db.Query("Update USERS SET Email=?, Name=?, Gender=?, BirthYear=? where Email=?",
		userTemp.Email, userTemp.Name, userTemp.Gender, userTemp.BirthYear, ID)
	if err != nil {
		panic(err.Error())
	}
	...
	newUser.gender = userTemp.Gender
	newUser.birthYear = userTemp.BirthYear
